...
public class ProductService {
	...
	public List<ProductSummaryDTO> getAllProducttWithPriceRangeByCategoryIds(List<Long> categoryIds, double lowPrice, double highPrice, int start, int max) {
	...
	public int getProductCountWithPriceRangeByCategoryIds(List<Long> categoryIds, double lowPrice, double highPrice) {
		String queryString = "select count(p.id) from Product p, Category c where c.id in :id and p.status = 'ACTIVE' and p.actualPrice between :lowPrice and :highPrice and c member of p.categories";
		TypedQuery<Long> count = em.createQuery(queryString, Long.class);
		//List<Long> list = Arrays.asList(49L,108L);
		List<Long> list = categoryIds;
		count.setParameter("id", list);
